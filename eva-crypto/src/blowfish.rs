...
impl BlowFish {
    ...
    fn f(&self, x: u32) -> u32 {
        let h = self.sbox[0][(x >> 24) as usize]
            .overflowing_add(self.sbox[1][(x >> 16 & 0xff) as usize])
            .0;
        (h ^ self.sbox[2][(x >> 8 & 0xff) as usize])
            .overflowing_add(self.sbox[3][(x & 0xff) as usize])
            .0
    }
}

pub static SBOX: [[u32; 256]; 4] = [
    [
